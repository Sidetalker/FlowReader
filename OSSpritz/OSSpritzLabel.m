...
- (void)setUp
{
    ...
    for (int i = 0; i < 18; i++)
    {
        ...
        if (i==pivotChar) label.textColor = [UIColor whiteColor]; // CUSTOM
        else label.textColor = [UIColor blackColor]; // CUSTOM
        label.alpha = 0;
        label.text = @"m";
...
//    NSArray *paths = @[upperLinePath, upperGuidePath, downGuidePath, downLinePath];
    ...
    for (UIBezierPath *path in paths)
    {
        ...
        line.fillColor = [UIColor blackColor].CGColor; // CUSTOMIZE
        line.opacity = 1.0;
...
- (void)drawBGBox
{
    ...
    boxLayer.fillColor = [UIColor colorWithRed:(147/255.0) green:(112/255.0) blue:(219/255.0) alpha:1].CGColor;
